...
class DiabloBalanceGraspClaude(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id,
        ...
        self.locked_h_tgt    = torch.zeros(n, dtype=torch.float32, device=dev)
        self.release_timer   = torch.zeros(n, dtype=torch.float32, device=dev)
    ...
    def compute_reward(self):
        (
            ...
            locked_h_tgt       = self.locked_h_tgt,
    ...
    def reset_idx(self, env_ids):
        ...
        self.locked_h_tgt[env_ids]        = 0.0
        self.release_timer[env_ids]       = 0.0
    ...
    def pre_physics_step(self, actions):
        ...
        # Dynamic Physical Height Lock: 
        # In Phase 3 (Release), force height to the SPECIFIC value captured at the moment of transition.
        is_releasing = (self.phase_buf == self.PHASE_RELEASE)
        h_tgt_policy = self.h_mid + h_cmd * self.h_range
        h_tgt = torch.where(is_releasing, self.locked_h_tgt, h_tgt_policy)
        h_tgt = torch.clamp(h_tgt, self.h_min, self.h_max)
        ...
        # Release Timer Logic: Check if object is centered and very close to platform
        obj_pos = self.states["object_pos"]
        plat_pos = self.platform_pos_tensor
        plat_surface_z = plat_pos[:, 2] + 0.005
        obj_up = quat_apply(self.states["object_rot"], torch.tensor([0., 0., 1.], device=self.device).repeat(N, 1))
        obj_bottom_z = obj_pos[:, 2] - self.object_half_heights * obj_up[:, 2]
        
        dist_xy_to_plat = torch.norm(obj_pos[:, :2] - plat_pos[:, :2], dim=-1)
        dist_z_to_plat  = obj_bottom_z - plat_surface_z
        
        # Condition: within 2cm XY and 1.0cm Z
        on_plat_stable = (dist_xy_to_plat < 0.02) & (torch.abs(dist_z_to_plat) < 0.01)
        
        # LATCHED Timer Logic:
        # 1. If stable now: increment timer
        # 2. If NOT stable now BUT already triggered (>60): keep the timer (LATCH)
        # 3. If NOT stable now AND NOT triggered: reset to 0 (must be consecutive 1s)
        self.release_timer = torch.where(
            on_plat_stable, 
            self.release_timer + 1,
            torch.where(self.release_timer >= 60, self.release_timer, torch.zeros_like(self.release_timer))
        )
        ...
        # SPECIAL: Force FULL OPEN if release_timer > 60 (1 second at 60Hz)
        ...
        forced_release = (self.release_timer > 60)
        ...
        # OVERRIDE: If forced_release is active, ignore everything and force open
        should_close = torch.where(forced_release, torch.zeros_like(should_close), should_close)
    ...
    def post_physics_step(self):
        ...
        # Capture the CURRENT commanded leg height at the moment of transition
        if ready_release.any():
            current_h_cmd = self.actions[ready_release, 0]
            self.locked_h_tgt[ready_release] = self.h_mid + current_h_cmd * self.h_range
...
def compute_balance_grasp_reward(
    ...
    platform_pos, phase_buf, locked_h_tgt,
    ...
    # Phase 0: h_mid (0.45)
    # Phase 1, 2, 3: h_grasp_low (0.42) - UNIFIED to prevent jumps during release and retreat
    
    is_approaching = (phase_buf == 0)
    is_manipulating = (phase_buf >= 1)
    
    h_grasp_low = 0.42
    # Ensure Phase 3 target matches the physical lock (locked_h_tgt is leg height, add offset)
    height_target = torch.where(is_approaching, torch.full_like(base_pos[:, 2], h_mid), 
                    ...
                    locked_h_tgt + 0.16)) # 0.16 is BODY_OFFSET
